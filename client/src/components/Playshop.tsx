  ...
  return (
    <section id="playshop" className="relative py-32 overflow-hidden bg-secondary/10">
      ...
      <div className="absolute inset-0 z-0">
        <img src="/assets/generated_images/abstract_creative_workshop_background.png" alt="Playshop Abstract" className="w-full h-full object-cover opacity-10" />
      ...
      <div className="container px-6 mx-auto relative z-10">
        <div className="text-center mb-16">
          ...
          <h2 className="font-serif text-5xl md:text-7xl text-foreground">Creative Rejuvenation Playshop</h2>
        </div>
        ...
        <div className="max-w-4xl mx-auto bg-background/80 backdrop-blur-md p-8 md:p-12 rounded-2xl border border-border shadow-sm mb-20 text-center">
          ...
          <div className="bg-secondary/20 rounded-xl p-6 mt-8 text-left">
            ...
            <p className="text-muted-foreground mb-4 text-center">Each playshop is tailored to your group's needs. Options include:</p>
            <ul className="space-y-2 mb-6 max-w-md mx-auto">
              <li className="flex items-center gap-2 text-sm text-foreground"><div className="w-1.5 h-1.5 rounded-full bg-primary" /> Small Session (1-2 hours)</li>
              <li className="flex items-center gap-2 text-sm text-foreground"><div className="w-1.5 h-1.5 rounded-full bg-primary" /> Full Day Experience</li>
              <li className="flex items-center gap-2 text-sm text-foreground"><div className="w-1.5 h-1.5 rounded-full bg-primary" /> Full Day with Live Music</li>
            </ul>
            <div className="text-center">
              <a href="#contact?type=playshop" className="inline-flex items-center gap-2 px-6 py-3 bg-primary text-primary-foreground font-medium rounded-full hover:bg-primary/90 transition-colors uppercase tracking-wider text-xs" data-testid="button-inquire-playshop">
                <Mail className="w-4 h-4" /> Email for Details & Booking
              </a>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto bg-gradient-to-br from-primary/10 to-secondary/20 backdrop-blur-md p-8 md:p-12 rounded-2xl border border-primary/20 shadow-lg mb-20">
          <div className="flex items-center gap-3 mb-6">
            <div className="w-12 h-12 rounded-full bg-primary/20 flex items-center justify-center"><Lightbulb className="w-6 h-6 text-primary" /></div>
          ...
          <div className="prose prose-lg text-muted-foreground leading-relaxed mb-6">
            <p className="mb-4">Unlock your creative potential through personalized one-on-one coaching sessions. Whether you're an artist seeking to break through creative blocks, a professional looking to integrate creativity into your work, or simply someone wanting to reconnect with your creative spirit, Celeste offers tailored guidance to help you flourish.</p>
            <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6 list-none pl-0">
              <li className="flex items-center gap-2 text-sm font-medium"><div className="w-1.5 h-1.5 rounded-full bg-primary" /> Creative Block Resolution</li>
              <li className="flex items-center gap-2 text-sm font-medium"><div className="w-1.5 h-1.5 rounded-full bg-primary" /> Artistic Vision Development</li>
              <li className="flex items-center gap-2 text-sm font-medium"><div className="w-1.5 h-1.5 rounded-full bg-primary" /> Mindfulness & Flow States</li>
              <li className="flex items-center gap-2 text-sm font-medium"><div className="w-1.5 h-1.5 rounded-full bg-primary" /> Work-Life Creative Balance</li>
            </ul>
          </div>
        </div>
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="bg-background border border-primary/20 rounded-2xl p-10 shadow-lg relative min-h-[250px] flex flex-col justify-center items-center text-center">
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <p className="font-serif text-2xl md:text-3xl text-foreground mb-6 leading-tight">"{quotes[currentQuote].text}"</p>
                <p className="text-primary font-medium uppercase tracking-wider text-sm">— {quotes[currentQuote].author}</p>
              </motion.div>
            </AnimatePresence>

            {/* Navigation */}
            <div className="flex gap-4 mt-8 absolute -bottom-6 left-1/2 -translate-x-1/2">
              <Button variant="default" size="icon" onClick={() => setCurrentQuote((prev) => (prev - 1 + quotes.length) % quotes.length)} className="rounded-full w-12 h-12 shadow-md">
                <ChevronLeft className="w-5 h-5" />
              </Button>
              <Button variant="default" size="icon" onClick={() => setCurrentQuote((prev) => (prev + 1) % quotes.length)} className="rounded-full w-12 h-12 shadow-md">
